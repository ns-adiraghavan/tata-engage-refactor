          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <div className="text-sm font-black text-slate-900 uppercase tracking-widest">{trigger.type}</div>
                <div className="text-[10px] text-slate-400 font-mono">{trigger.condition}</div>
              </div>
            </div>
                ...
                <div className="p-8 space-y-6">
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Send Timing</label>
                        <select 
                    ...
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Email Body (Merge Tags: {"{{volunteer_name}}"}, {"{{project_title}}"})</label>
                      <textarea 
                  ...
                  <div className="pt-6 border-t border-slate-200 flex justify-end gap-4">
                    <button 
                      ...
                      className="px-6 py-2 border border-slate-200 text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all flex items-center gap-2"
                    >
                      <Send size={14} /> Send Test Email
                    </button>
                    <button className="px-6 py-2 bg-tata-blue text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-900 transition-all">
                      Save Template
                    </button>
                  </div>
                </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-8">
        <div className="bg-white border border-slate-200 p-8 space-y-6">
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest border-b border-slate-100 pb-4">Compose Bulk Dispatch</h3>
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {COMPANY_DOMAINS.map(c => (
                  <button 
                    ...
                    className={`p-3 border text-left text-[10px] font-bold uppercase tracking-widest transition-all ${
            ...
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Email Subject</label>
              <input 
                type="text" 
          ...
          <div className="pt-6 border-t border-slate-100 flex justify-end gap-4">
            <button 
              ...
              className="px-8 py-3 border border-slate-200 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2"
            >
            ...
            <button 
              ...
              className="px-8 py-3 bg-zinc-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all disabled:opacity-50"
            >
              Submit for Send
            </button>
          </div>
      ...
      <div className="space-y-6">
        <div className="bg-white border border-slate-200">
          <div className="p-6 border-b border-slate-200">
            <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Dispatch History</h3>
          </div>
              ...
              <div key={email.id} className="p-4 space-y-2 hover:bg-slate-50 transition-colors">
                ...
                {email.status === "Sent" && (
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <span className="text-[10px] font-black text-tata-blue">{email.openRate} Open</span>
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="bg-blue-50 border border-blue-100 p-6">
          <div className="flex items-center gap-3 text-tata-blue mb-4">
            <ShieldCheck size={20} />
            <h4 className="text-[10px] font-black uppercase tracking-widest">Approval Gate Active</h4>
          </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-10 bg-black/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-2xl shadow-2xl border border-slate-200 flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Email Preview Render</h3>
            ...
            <div className="p-10 flex-1 overflow-y-auto space-y-8 bg-white">
              <div className="border-b border-slate-100 pb-4">
                ...
                <div className="text-lg font-black text-slate-900">{bulkSubject || "(No Subject)"}</div>
              </div>
              <div className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">
                {bulkBody || "(No Body Content)"}
              </div>
              <div className="pt-10 border-t border-slate-100 text-center">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Footer Preview</div>
                ...
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-slate-900">{p.feedbackPercent}%</span>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right space-x-4">
                    ...
                    <button 
                      ...
                      className="text-[10px] font-black text-tata-blue uppercase tracking-widest hover:underline disabled:opacity-30"
                    >
      ...
      {showTriggerModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
          <div className="bg-white p-8 w-full max-w-md border border-slate-200 shadow-2xl text-center">
            ...
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-4">Trigger Certificates?</h3>
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowTriggerModal(null)} className="flex-1 py-3 border border-slate-200 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
              <button 
      ...
      {viewVolunteers && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl border border-slate-200 shadow-2xl flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Manage Project Volunteers</h3>
                <p className="text-[10px] text-slate-400 font-mono">{viewVolunteers.title}</p>
              </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {viewVolunteers.volunteers.map((v: any) => (
                    <tr key={v.id}>
                      ...
                      <td className="p-4">
                        <span className="px-2 py-0.5 bg-green-100 text-green-600 text-[8px] font-black uppercase tracking-widest rounded">{v.status}</span>
                      </td>
