        ...
        {items.filter(i => i.status === status).map(item => (
          <div key={item.id} className="bg-white p-4 border border-slate-200 shadow-sm hover:border-tata-blue transition-all group">
            <div className="flex justify-between items-start mb-2">
              <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded uppercase">{item.type}</span>
            ...
            <div className="flex flex-wrap gap-1 mb-4">
              {item.tags.map(t => <span key={t} className="text-[10px] font-bold text-tata-blue">#{t}</span>)}
            </div>
            <div className="flex justify-between items-center pt-4 border-t border-slate-50">
              <div className="text-[10px] text-slate-400 font-mono">By {item.author}</div>
              <div className="flex gap-2">
                {status === "Draft" && (
                  <button onClick={() => moveItem(item.id, "In Review")} className="text-[10px] font-semibold text-tata-blue uppercase tracking-widest hover:underline">Submit</button>
                )}
                {status === "In Review" && (
                  <>
                    <button onClick={() => moveItem(item.id, "Published")} className="text-[10px] font-semibold text-green-600 uppercase tracking-widest hover:underline">Publish</button>
                    <button onClick={() => moveItem(item.id, "Draft")} className="text-[10px] font-semibold text-red-600 uppercase tracking-widest hover:underline">Return</button>
                  </>
                )}
                {status === "Published" && (
                  <button onClick={() => moveItem(item.id, "Draft")} className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest hover:underline">Unpublish</button>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6 flex-1 overflow-y-auto pr-4">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={handleAiSuggest} disabled={!newItem.title || isAiLoading} className="text-xs font-semibold text-tata-blue uppercase tracking-widest flex items-center gap-1 hover:underline disabled:opacity-30">
