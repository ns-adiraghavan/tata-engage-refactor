...
export const CMSContentPanel = ({ addAuditLog, triggerToast }: { addAuditLog: any, triggerToast: any }) => {
  ...
  const Column = ({ title, status }: { title: string, status: string }) => (
    <div className="flex-1 bg-slate-50 p-6 border border-slate-200 min-h-[600px]">
      <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-200 pb-2">{title}</h3>
        ...
        {items.filter(i => i.status === status).map(item => (
          <div key={item.id} className="bg-white p-4 border border-slate-200 shadow-sm hover:border-tata-blue transition-all group">
            <div className="flex justify-between items-start mb-2">
              <span className="text-[8px] font-black px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded uppercase">{item.type}</span>
              <span className="text-[8px] font-mono text-slate-400">v{item.version}</span>
            </div>
            <h4 className="text-xs font-bold text-slate-900 mb-2">{item.title}</h4>
            <div className="flex flex-wrap gap-1 mb-4">
              {item.tags.map(t => <span key={t} className="text-[8px] font-bold text-tata-blue">#{t}</span>)}
            </div>
            <div className="flex justify-between items-center pt-4 border-t border-slate-50">
              <div className="text-[8px] text-slate-400 font-mono">By {item.author}</div>
              <div className="flex gap-2">
                {status === "Draft" && (
                  <button onClick={() => moveItem(item.id, "In Review")} className="text-[8px] font-black text-tata-blue uppercase tracking-widest hover:underline">Submit</button>
                )}
                {status === "In Review" && (
                  <>
                    <button onClick={() => moveItem(item.id, "Published")} className="text-[8px] font-black text-green-600 uppercase tracking-widest hover:underline">Publish</button>
                    <button onClick={() => moveItem(item.id, "Draft")} className="text-[8px] font-black text-red-600 uppercase tracking-widest hover:underline">Return</button>
                  </>
                )}
                {status === "Published" && (
                  <button onClick={() => moveItem(item.id, "Draft")} className="text-[8px] font-black text-slate-400 uppercase tracking-widest hover:underline">Unpublish</button>
                )}
              </div>
            </div>
          </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        ...
        <button 
          ...
          className="px-6 py-3 bg-tata-blue text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-tata-blue/20 hover:bg-blue-900 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Column title="In Review" status="In Review" />
        <Column title="Published" status="Published" />
      </div>

      <AnimatePresence>
        {showDrawer && (
          <div className="fixed inset-0 z-[200] flex justify-end bg-black/50 backdrop-blur-sm">
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col p-10">
              <div className="flex justify-between items-center mb-10">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Create New Content</h3>
              ...
              <div className="space-y-6 flex-1 overflow-y-auto pr-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Content Type</label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={handleAiSuggest} disabled={!newItem.title || isAiLoading} className="text-[10px] font-black text-tata-blue uppercase tracking-widest flex items-center gap-1 hover:underline disabled:opacity-30">
                      {isAiLoading ? <Activity size={12} className="animate-spin" /> : <Sparkles size={12} />} Suggest Draft (AI-06)
                    </button>
                  </div>
                  <textarea className="w-full p-4 border border-slate-200 outline-none focus:border-tata-blue text-sm min-h-[200px] leading-relaxed" placeholder="Write content here..." value={newItem.body} onChange={e => setNewItem({...newItem, body: e.target.value})} />
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    {["TVW", "ProEngage", "General"].map(tag => (
                      <button key={tag} onClick={() => setNewItem({...newItem, tags: newItem.tags.includes(tag) ? newItem.tags.filter(t => t !== tag) : [...newItem.tags, tag]})} className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest border transition-all ${newItem.tags.includes(tag) ? 'bg-tata-blue text-white border-tata-blue' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                        {tag}
                      </button>
                    ))}
                  </div>
              ...
              <div className="pt-10 border-t border-slate-100 flex gap-4">
                <button onClick={() => setShowDrawer(false)} className="flex-1 py-4 border border-slate-200 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Discard</button>
                <button onClick={() => { triggerToast("Draft saved."); setShowDrawer(false); }} className="flex-1 py-4 bg-tata-blue text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-900 transition-all">Save as Draft</button>
              </div>
            </motion.div>
          </div>
        )}
